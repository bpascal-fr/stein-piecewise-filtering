...
%        u,e
%   
%   where   - z in R^{LM} is the initial degraded signal (created from SIGNAL)
...
%
%   RES = DMS(SIGNAL, BETA, LAMBDA) returns a structure containing the input
...
%       RES.u            : the resulting u
%       RES.e            : the resulting e
...
%
%   RES = DMS(SIGNAL, BETA, LAMBDA, NAME1, VAL1,...) solves the Discrete 
...
% 
%   S = [-2*(0:.01:2)+3, (2:.01:3).^2-1, 2*(3:.01:5)-5];
% 
%   res = dms_1D(S,10,0.1,'AddNoise',[1 0.1]);
%
%   figure, colormap gray
%   plot(res.ground_truth,'linewidth',2); hold on;
%   plot(res.data,'linewidth',2);        
%   plot(res.u,'linewidth',2); 
%   plot(res.e,'*','markersize',10); hold off;
%   legend('ground truth','data $z$','estimate $u$','jump set $e$')
%
%   Class Support
%   -------------
...
% save parameters
p = {'bet','lam','alp','reg','init','blur','noise','edges','N','eps'};
params = cell2struct({bet,lam,alp,reg,init,blur,noise,edges,N,eps},p,2);
...
[J,L,S,R] = energy(params,norm,prox);
...
function [J,L,S,R] = energy(params,norm,prox)
...
switch params.noise.type
    case {'none','Gaussian'}
        if strcmp(params.blur.type,'none')
            L.expr = @(u,z)     0.5*norm.L2(u-z).^2;
            L.prox = @(x,tau,z) prox.L2_dist_z(x,tau,z);
        else
            L.expr = @(u,z)     0.5*norm.L2(A(u)-z).^2;
            L.prox = @(x,tau,z) prox.L2_dist_z_resto(x,tau,z,matA);
        end
    case 'Poisson'
...
switch params.reg
    ...
    case 'l1'
        ...
        R.prox = prox.L1;
    case 'l1q'
        R.expr = @(e) norm.quadL1(e);
        R.prox = prox.quadL1;
end

% S: smoothing term, preserving edges
S.expr = @(u,e) mat_sum(((1-e).*D(u)).^2);
 
S.D_sq = @(u) D(u).^2;

S.du = @(u,e)  2*Dadj((1-e).^2.*D(u)); 
...
switch params.init.u
    ...
    case 'rand_cst', u0 = randn*ones(size(z));
    case 'data',     u0 = z;
end

switch params.init.edges
    case '1',        e0 = ones([1 length(z)-1]);
    case '0',        e0 = zeros([1 length(z)-1]);
    case 'binomial', e0 = binornd(1,0.5,[1 length(z)-1]);
    case 'rand_cst'
        tmp = abs(0.25*randn);
        while tmp > 1., tmp = abs(0.25*randn); end
        e0 = tmp*ones([1 length(z)-1]);
end

end

function norm = norms(eps)

mat_sum = @(M) sum(M(:));

% primal norms
norm.L0      = @(x) sum(double(x(:) ~= 0));

norm.L1      = @(x) sum(abs(x(:))); 
norm.quadL1  = @(x) mat_sum(max(abs(x(:)),x(:).*x(:)./(4*eps)));   

norm.L2      = @(x) sqrt(sum(x(:).^2)); 
norm.L2sq    = @(x) sum(x(:).^2); 


% dual norms
%norm.L2_dual = @(y) ( sqrt(sum(y.^2,3)) );
...
prox.quadL1 = @(x,tau) max(0,min(abs(x)-tau,max(4*eps,abs(x)./(tau/(2*eps)+1)))).*sign(x);
...
prox.L2_dist_z       = @(x,tau,z)      (x+2*tau.*z)./(1+2*tau);   
prox.L2_dist_z_resto = @(x,tau,z,matA) ...
    real(ifft2((fft2(x)+2*tau*conj(matA).*fft2(z))./(2*tau*conj(matA).*matA + 1))); 
...
y1 = [x(1), x(2:end)-x(1:end-1), -x(end)]./2.;
y  = - y1;
...
switch blur.type
    case 'none'
        F = 1.;
    case 'Gaussian'
        fprintf(1,'Adding %s blur of size %i with std %g ...\n',...
                                blur.type,blur.size,blur.std);
        fs  = (blur.size-1)/2;
        x   = -fs:fs;
        arg = -(x.*x)/(2*blur.std*blur.std);
        
        F = exp(arg);
        %F(F<eps*max(F(:))) = 0;
        F = F./sum(F(:));
    case 'Uniform'
        fprintf(1,'Adding %s blur of size %i ...\n',...
                               blur.type,blur.size);
        F = ones(1,blur.size,'double');
        F = F/norm(F(:));
end

blur_out.mat = psf2otf(F,[h,w]); 
blur_out.mat = repmat(blur_out.mat,[1 1 c]);
blur_out.op  = @(x) real(ifft(blur_out.mat.*fft(x)));  
...
if (ndims(im) > 2) || (ndims(im) == 2 && prod(size(im)) > max(size(im)))
    error('SIGNAL cannot have more than 1 dimensions.');
end
