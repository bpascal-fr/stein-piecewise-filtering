function x = opLadj_auto(y,filter_def, computation, param, lambda)
        ...
        if strcmp(filter_def,'gradient')
            if numel(lambda) == 1
                if strcmp(computation,'fourier')
                    ...
                    x = -lambda*([y(:,1:1)/2,y(:,2:dim(2)-1)/2- y(:,1:dim(2)-2)/2,-y(:,dim(2)-2+1:dim(2)-1)/2]);
                end
            else
                    x = -diag(lambda)*([y(:,1:1)/2,y(:,2:dim(2)-1)/2- y(:,1:dim(2)-2)/2,-y(:,dim(2)-2+1:dim(2)-1)/2]);
            end
        elseif strcmp(filter_def,'laplacian')
            if numel(lambda) == 1
                if strcmp(computation,'fourier')
                    ...
                    x = lambda*([y(:,1:1)/4,-y(:,1)/2 + y(:,2)/4, y(:,3:dim(2)-2)/4 - y(:,2:dim(2)-3)/2 + y(:,1:dim(2)-4)/4, y(:,dim(2)-3)/4 - y(:,dim(2)-2)/2, y(:,dim(2)-2)/4]);
                end
            else
                x = diag(lambda)*([y(:,1:1)/4,-y(:,1)/2 + y(:,2)/4, y(:,3:dim(2)-2)/4 - y(:,2:dim(2)-3)/2 + y(:,1:dim(2)-4)/4, y(:,dim(2)-3)/4 - y(:,dim(2)-2)/2, y(:,dim(2)-2)/4]);
    ...
    if ischar(filter_def)
        if strcmp(filter_def,'gradient')
            ...
            if strcmp(computation,'fourier')
                    ...
                    yt = y(:,:,1);
                    [~,m] = size(yt);
                    x = -lambda(1)*([yt(:,1:tau)/2,yt(:,1+tau:m-tau)/2- yt(:,1:m-2*tau)/2,-yt(:,m-2*tau+1:m-tau)/2]);
                    yt = y(:,:,2)';
                    [~,m] = size(yt);
                    x = x -lambda(2)*([yt(:,1:tau)/2,yt(:,1+tau:m-tau)/2- yt(:,1:m-2*tau)/2,-yt(:,m-2*tau+1:m-tau)/2])';
            end
